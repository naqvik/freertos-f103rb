...
inline void enable_afio_clk(void) {
    RCC->APB2ENR |= (1u << 0); // bits[0] = AFIOEN <- 1
}
inline void enable_gpioa_clk(void) {
    RCC->APB2ENR |= (1u << 2); // bits[2] = IOPAEN <- 1
}
inline void enable_gpiob_clk(void) {
    RCC->APB2ENR |= (1u << 3); // bits[3] = IOPAEN <- 1
}
inline void enable_gpioc_clk(void) {
    RCC->APB2ENR |= (1u << 4); // bits[4] = IOPCEN <- 1
}
inline void enable_gpiod_clk(void) {
